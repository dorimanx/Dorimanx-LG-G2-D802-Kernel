...
RAMDISK_BRANCH=$(git -C ../LG-G2-D802-Ramdisk/ commit | grep "origin/master" | wc -l);
...
if [ "$RAMDISK_BRANCH" == "1" ]; then
	...
	if [ "$RAMDISK_NOT_SAVED" != "1" ]; then
		...
		echo -e "\e[1;31mRamDisk Switched to master Branch!\e[m"
	else
		echo -e "\e[1;31mRamDisk has to be SAVED (commited) before switching to Master Branch\e[m";
