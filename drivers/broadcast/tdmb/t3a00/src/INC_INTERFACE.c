...
INC_UINT16 INC_I2C_READ(INC_UINT8 ucI2CID, INC_UINT16 uiAddr)
{
	...
	AckStatus= tdmb_t3a00_i2c_read_burst(uiAddr, acBuff,2);	//for android compile
	if(AckStatus == TRUE){
		wData = ((INC_UINT16)acBuff[0] << 8) | (INC_UINT16)acBuff[1];
		return wData;
	}
...
INC_UINT8 INC_I2C_WRITE(INC_UINT8 ucI2CID, INC_UINT16 uiAddr, INC_UINT16 uiData)
{
	...
	AckStatus = tdmb_t3a00_i2c_write_burst(uiAddr, acBuff, ucCnt);	//for android compile
	if(AckStatus == INC_SUCCESS)
	{
		return INC_SUCCESS;
	}
...
INC_UINT16 INC_SPI_REG_READ(INC_UINT8 ucI2CID, INC_UINT16 uiAddr)
{
	...
	INC_INTERRUPT_LOCK();
	//tdmb_t3a00_spi_write_read(&auiTxBuff[0], 4, &auiRxBuff[0], 2);
...
INC_UINT16 INTERFACE_ISR(INC_UINT8 ucI2CID, INC_UINT8* pBuff)
{
	...
	/* pBuffer size is 8120. Refer to  TDMB_MPI_BUF_SIZE defined in broadcast_t3a00_drv_if.c */
